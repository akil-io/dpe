...
class Client {
    ...
    upgrade(_api, _meta) {
        ...
        this._zone = _meta.zone ? _meta.zone : 0;
        this._uid = _meta.uid ? _meta.uid : null;
        this._port = _meta.port ? _meta.port : 49001;
        this._tags = _meta.tags ? _meta.tags : [];
        this._version = _meta.version ? _meta.version : '';
    ...
    close() {
        ...
        this._wait.forEach((v, k) => {
            clearTimeout(this._wait.get(k)._timer);
            this._wait.get(k)._cb(new Error('E_CLOSED'));
        });
    }
    getTimer(_ttl, _sid) {
        return setTimeout(() => {
    ...
    response(_signal) {
        debug('send response', this._cid, _signal._cid);
        if (_signal instanceof Error) {
            _signal = new Signal(_signal);
        }
