...
class Core {
    constructor(_service) {
        ...
        this.instance.set('dpe.connect', new API('dpe.connect', Core.connect));
        this.instance.set('dpe.disconnect', new API('dpe.connect', Core.disconnect));
        this.instance.set('instance.list', new API('core.list', Core.getInstanceList));
        this.instance.set('client.list', new API('core.list', Core.getClientList));
    }

    static getClientList(env, args, done) {
        var list = [];
        env.client.forEach((val, key) => list.push(key));
        done(null, list);
    }

    static getInstanceList(env, args, done) {
        var list = [];
        env.instance.forEach((val, key) => list.push(key));
        done(null, list);
    }
    ...
    static connect(env, args, done) {
        env.service.connect(args.url, args.port, (_err, _api) => {
            ...
            env.onClientConnected(`${args.url}:${args.port}`, _api);
            done(null, {
    ...
    onClientConnected(_cid, _api) {
        this.client.set(_cid, new Client(_cid, _api));
        ...
        switch (api.constructor.name) {
            ...
            case 'Component':
                ...
                return api.invoke(context, {
                    ...
                    get: this.service.get.bind(this.service),
