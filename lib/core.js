...
class Core {
    ...
    send(_signal) {
        if (_signal['@broadcast'] != undefined) {
            var targets = [];
            if (_.isArray(_signal['@broadcast'])) {
                this.client.forEach((v, k) => {
                    if (_.intersection(v._tags, _signal['@broadcast']).length == _signal['@broadcast'].length) {
                        targets.push(k);
                    }
                });
            }
            ...
            } else if (_.isString(_signal['@broadcast'])) {
                this.client.forEach((v, k) => {
                    var r = new RegExp(_signal['@broadcast'], 'ig');
            ...
            targets.map((cid) => {
                this.client.get(cid).request(_signal, (_response) => {
                    if (this.client.has(cid)) {
                        this.client.get(cid).response(_response);
                    }
                });
            });
            return;
