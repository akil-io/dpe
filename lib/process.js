    ...
    processIterator(name, signal) {
        ...
        async.doWhilst((next) => {
            this._api(this._plan[name]['iterate'], this._args[this._plan[name]['iterate']], (err, result) => {
                lastItem = err ? err : result;

                if (lastItem) {
                    this._api(name, lastItem, (err, result) => {
                        if (_.isPlainObject(result)) this._state[name] = Object.assign(this._state[name], result);
                    });
                } else next();
            });
        }, () => { (lastItem instanceof Error) || !lastItem }, () => {
            if (this._plan['@target'].indexOf(name) != -1) {
                this._ready.push(name);
            }
        });
