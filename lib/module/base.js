...
class BaseAPI {
    ...
    static updatePackage(env, args, done) {
        if (!args.name) return done(new Error('E_REQUIRED'));

        var s = env.service;
        var deps = env.service.get('deps');
        var packageName = args.name;
        var pack = env.service.get('deps.' + packageName);
        ...
        if (!pack) {
            ...
            var type = null;
            if (pack['file']) type = 'file';
            if (pack['git']) type = 'git';
            if (pack['npm']) type = 'npm';

            switch (type) {
                case "git":
                    const child1 = spawn('git', ['pull'], {
                        detached: true,
                        cwd: path.join(s.get('module.path'), packageName),
                        stdio: 'inherit'
                    });
                    child1.on('close', (code) => {
                        if (code != 0) {
                            debug('Can not update module to dependencies');
                            done(new Error('E_FAILED'));
                        } else {
                            const child2 = spawn(npmCmd, ['install'], {
                                detached: true,
                                cwd: path.join(s.get('module.path'), packageName),
                                stdio: 'inherit'
                            });
                    ...
                    break;
                case "npm":
                    done(new Error('E_NOT_SUPPORTED'));
                    break;
                case "file":
                    done(new Error('E_NOT_SUPPORTED'));
                    break;
    ...
    static discover(env, args, done) {
        if (!args.cid) return done(new Error('E_REQUIRED'));

        var _cid = args.cid;
        var _client = env.client.get(_cid);

        env.instance.forEach((api, key) => {
            api.from = _.compact(api.from.map((v) => env.client.has(v) ? v : null));

            api.from = _.without(api.from, _cid);
            if (api.from.length == 0 && !api.export && api.access != "core") {
                debug('remove: ' + key);
                env.instance.delete(key);
            }
        });

        //if (["cli", "local-cli"].indexOf(_client._type) == -1) {
        if (_client.type == "server" || _client._tags.indexOf("discover") != -1) {
            _client.request(new Signal({
                _cid: "self",
                _ttl: 10000,
                "instance.list": {
                    export: true
                ...
                if (!(_signal instanceof Signal)) {
                    debug(' +++++++ BAD ANSWER FROM CLIENT ++++++++', _signal);
                    return done(null, null);
                }
                var exportList = _signal.get('instance.list');
                if (exportList && exportList['$result']) exportList = exportList['$result'];
                debug("HANDLE REMOTE INSTANCES");
                exportList.map((item) => {
                    if (!env.instance.has(item.name)) {
                        debug('export: ' + item.name);
                        env.instance.set(item.name, new API(item.name, {
                                access: "module",
                                from: _cid
                            }, env.getRemoteCallAPI(item.name)
                        ));
                    } else {
                        env.instance.get(item.name).from.push(_cid);
                    }
                });
    ...
    static discoverAll(env, args, done) {
        async.eachSeries(env.client, function (clientItem, next) {
            var [_cid, _client] = clientItem;

            BaseAPI.discover(env, {
                cid: _cid
            }, next);
        }, done);
    }

    static ping(env, args, done) {
        done(null, "pong");
    }
}
...
module.exports = function (core, API) {
    core.instance.set(
        'core.discover',
        new API('core.discover', {access:"core"},  BaseAPI.discover)
    );
    core.instance.set(
        'core.rediscover',
        new API('core.rediscover', {access:"core"},  BaseAPI.discoverAll)
    );
    core.instance.set(
        'core.ping',
        new API('core.ping', {access:"core"},  BaseAPI.ping)
    );
    core.instance.set(
        'net.add',
    ...
    core.instance.set(
        'package.update',
        new API('package.update', {access:"core"},  BaseAPI.updatePackage)
    );
    core.instance.set(
        'package.list',
    ...
    core.instance.set(
        'core.set',
        new API('core.get', {access:"core"},  BaseAPI.setConfig)
    );
    core.instance.set(
        'core.restart',
    ...
    core.instance.set(
        'dpe.disconnect',
        new API('dpe.disconnect', {access:"core"}, BaseAPI.disconnect)
    );
    core.instance.set(
        'instance.list',
