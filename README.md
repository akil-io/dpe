...
DPE Framework является уровнем абстракции над задачами деплоя и масштабирования сложных вычислительных процессов. План вычислений записывается в JSON с возможностью задать индивидуальные условия исполнения конкретного процесса. Работает на базе Socket.IO.

В данный момент протокол DPE реализует одноименный пакет для Node.JS и расширение Chrome, для запуска процессов внутри Chrome.
